...
@interface rqdMediaRowController()
...
@implementation rqdMediaRowController
...
- (void)requestBackgroundImageForObjectID:(NSManagedObjectID *)objectID {
    NSDictionary *dict = [rqdMediaWatchMessage messageDictionaryForName:rqdMediaWatchMessageNameRequestThumbnail
                                                           payload:@{rqdMediaWatchMessageKeyURIRepresentation : objectID.URIRepresentation.absoluteString}];
...
- (void)backgroundThumbnailSetter:(NSArray *)array
{
    ...
    UIImage *backgroundImage = [rqdMediaThumbnailsCache thumbnailForManagedObject:managedObject
                                                                refreshCache:NO
