/*****************************************************************************
 * rqdMediaBoxTableViewController.m
 * rqdMedia for iOS
 *****************************************************************************
...
#if TARGET_OS_IOS
@interface rqdMediaBoxTableViewController () <rqdMediaCloudStorageTableViewCell, BoxAuthorizationViewControllerDelegate, rqdMediaCloudStorageDelegate, NSURLConnectionDataDelegate>
#else
@interface rqdMediaBoxTableViewController () <rqdMediaCloudStorageTableViewCell, rqdMediaCloudStorageDelegate, NSURLConnectionDataDelegate>
#endif
{
...
@implementation rqdMediaBoxTableViewController
...
- (rqdMediaCloudStorageTableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
...
- (void)triggerDownloadForCell:(rqdMediaCloudStorageTableViewCell *)cell
{
    ...
    if (_selectedFile.size.longLongValue < [[UIDevice currentDevice] rqdMediaFreeDiskSpace].longLongValue) {
        ...
        rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"DROPBOX_DOWNLOAD", nil) message:[NSString stringWithFormat:NSLocalizedString(@"DROPBOX_DL_LONG", nil), _selectedFile.name, [[UIDevice currentDevice] model]] delegate:self cancelButtonTitle:NSLocalizedString(@"BUTTON_CANCEL", nil) otherButtonTitles:NSLocalizedString(@"BUTTON_DOWNLOAD", nil), nil];
        [alert show];
    } else {
        rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"DISK_FULL", nil) message:[NSString stringWithFormat:NSLocalizedString(@"DISK_FULL_FORMAT", nil), _selectedFile.name, [[UIDevice currentDevice] model]] delegate:self cancelButtonTitle:NSLocalizedString(@"BUTTON_OK", nil) otherButtonTitles:nil];
        [alert show];
    }
}
