/*****************************************************************************
 * rqdMediaHTTPUploaderController.m
 * rqdMedia for iOS
 *****************************************************************************
...
#import "rqdMediaHTTPConnection.h"
...
@interface rqdMediaHTTPUploaderController()
{
...
@implementation rqdMediaHTTPUploaderController

+ (instancetype)sharedInstance
{
    static rqdMediaHTTPUploaderController *sharedInstance = nil;
    static dispatch_once_t pred;

    dispatch_once(&pred, ^{
        sharedInstance = [rqdMediaHTTPUploaderController new];
    });
...
- (id)init
{
    if (self = [super init]) {
        ...
        BOOL isHTTPServerOn = [[NSUserDefaults standardUserDefaults] boolForKey:krqdMediaSettingSaveHTTPUploadServerStatus];
...
- (void)applicationDidBecomeActive: (NSNotification *)notification
{
    if (!_httpServer.isRunning)
        [self changeHTTPServerState:[[NSUserDefaults standardUserDefaults] boolForKey:krqdMediaSettingSaveHTTPUploadServerStatus]];
    ...
    if (_httpServer.isRunning) {
        if (!_backgroundTaskIdentifier || _backgroundTaskIdentifier == UIBackgroundTaskInvalid) {
            ...
            if ([[UIApplication sharedApplication] respondsToSelector:@selector(beginBackgroundTaskWithName:expirationHandler:)]) {
                _backgroundTaskIdentifier = [[UIApplication sharedApplication] beginBackgroundTaskWithName:@"rqdMediaUploader" expirationHandler:expirationHandler];
            } else {
...
- (BOOL)changeHTTPServerState:(BOOL)state
{
    ...
    [_httpServer setIPv6Enabled:[[[NSUserDefaults standardUserDefaults] objectForKey:krqdMediaSettingWiFiSharingIPv6] boolValue]];
    ...
    [_httpServer setConnectionClass:[rqdMediaHTTPConnection class]];
...
- (void)cleanCache
{
    if ([[rqdMediaActivityManager defaultManager] haveNetworkActivity])
        return;
