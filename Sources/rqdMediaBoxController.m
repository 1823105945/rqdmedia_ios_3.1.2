...
@interface rqdMediaBoxController ()
{
...
@implementation rqdMediaBoxController

#pragma mark - session handling

+ (rqdMediaCloudStorageController *)sharedInstance
{
    static rqdMediaBoxController *sharedInstance = nil;
    static dispatch_once_t pred;

    dispatch_once(&pred, ^{
        sharedInstance = [rqdMediaBoxController new];
    });
...
- (void)startSession
{
    ...
    [BoxSDK sharedSDK].OAuth2Session.clientID = krqdMediaBoxClientID;
    [BoxSDK sharedSDK].OAuth2Session.clientSecret = krqdMediaBoxClientSecret;

    NSString *token = [XKKeychainGenericPasswordItem itemForService:krqdMediaBoxService account:krqdMediaBoxAccount error:nil].secret.stringValue;
    if (!token) {
        NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
        [ubiquitousStore synchronize];
        token = [ubiquitousStore stringForKey:krqdMediaStoreBoxCredentials];
    }
    if (token != nil) {
...
- (void)logout
{
    ...
    NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
    [ubiquitousStore setString:nil forKey:krqdMediaStoreBoxCredentials];
    [ubiquitousStore synchronize];
    [self stopSession];
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:rqdMediaBoxControllerSessionUpdated object:nil];
}
...
#pragma mark - rqdMedia internal communication and delegate
