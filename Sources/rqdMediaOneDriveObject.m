...
#if TARGET_OS_IOS
@interface rqdMediaOneDriveObject () <rqdMediaHTTPFileDownloader>
{
    rqdMediaHTTPFileDownloader *_fileDownloader;
}

@end
#else
@interface rqdMediaOneDriveObject ()

@end
#endif

@implementation rqdMediaOneDriveObject
...
- (BOOL)hasFullFolderTree
{
    ...
    if (self.folders != nil) {
        ...
        for (NSUInteger x = 0; x < count; x++) {
            rqdMediaOneDriveObject *folder = self.folders[x];
...
- (void)liveOperationSucceeded:(LiveDownloadOperation *)operation
{
    ...
    if ([userState isEqualToString:@"load-folder-content"]) {
        ...
        for (NSUInteger x = 0; x < count; x++) {
            ...
            rqdMediaOneDriveObject *oneDriveObject = [[rqdMediaOneDriveObject alloc] init];
...
{
#if TARGET_OS_IOS
    _fileDownloader = [[rqdMediaHTTPFileDownloader alloc] init];
