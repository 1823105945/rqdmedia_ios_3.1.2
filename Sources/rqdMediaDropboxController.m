...
@interface rqdMediaDropboxController ()
{
...
@implementation rqdMediaDropboxController

#pragma mark - session handling

+ (instancetype)sharedInstance
{
    static rqdMediaDropboxController *sharedInstance = nil;
    static dispatch_once_t pred;

    dispatch_once(&pred, ^{
        sharedInstance = [rqdMediaDropboxController new];
...
- (void)shareCredentials
{
    ...
    [ubiquitousStore setArray:credentials forKey:krqdMediaStoreDropboxCredentials];
...
- (BOOL)restoreFromSharedCredentials
{
    NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
    [ubiquitousStore synchronize];
    NSArray *credentials = [ubiquitousStore arrayForKey:krqdMediaStoreDropboxCredentials];
    if (!credentials)
        return NO;
    for (NSString *tmp in credentials) {
        [DBSDKKeychain storeValueWithKey:krqdMediaStoreDropboxCredentials value:tmp];
    }
    return YES;
}
...
{
#if TARGET_OS_IOS
    rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"ERROR_NUMBER", nil), error.code]
