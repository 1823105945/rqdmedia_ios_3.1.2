...
@interface rqdMediaHTTPFileDownloader () <NSURLSessionDelegate>
{
    NSString *_filePath;
...
@implementation rqdMediaHTTPFileDownloader
...
- (void)downloadFileFromURL:(NSURL *)url withFileName:(NSString*)fileName
{
    ...
    if (_fileName.pathExtension.length == 0 || ![_fileName isSupportedFormat]) {
        _fileName = [_fileName stringByAppendingPathExtension:@"rqdmedia"];
    }
    ...
    [theRequest addValue:[NSString stringWithFormat:@"Mozilla/5.0 (%@; CPU OS 7_0 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/%@ Mobile/11A465 Safari/9537.53 rqdMedia for iOS/%@", UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? @"iPad" : @"iPhone", [[UIDevice currentDevice] systemVersion], [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"]] forHTTPHeaderField:@"User-Agent"];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    if (_statusCode == 200) {
        ...
        if (_expectedDownloadSize  > [[UIDevice currentDevice] rqdMediaFreeDiskSpace].longLongValue) { //handle too big a download
