...
#import "rqdMediaOneDriveConstants.h"
...
@interface rqdMediaOneDriveController () <LiveAuthDelegate, rqdMediaOneDriveObjectDelegate, rqdMediaOneDriveObjectDownloadDelegate>
{
...
@implementation rqdMediaOneDriveController

+ (rqdMediaCloudStorageController *)sharedInstance
{
    static rqdMediaOneDriveController *sharedInstance = nil;
    static dispatch_once_t pred;

    dispatch_once(&pred, ^{
        sharedInstance = [[rqdMediaOneDriveController alloc] init];
    });
...
- (void)setupSession
{
    ...
    _liveClient = [[LiveConnectClient alloc] initWithClientId:krqdMediaOneDriveClientID
                                                       scopes:_liveScopes
                                                     delegate:self
                                                    userState:@"init"];
}
...
- (void)logout
{
    ...
    [ubiquitousStore removeObjectForKey:krqdMediaStoreOneDriveCredentials];
...
- (void)authCompleted:(LiveConnectSessionStatus)status session:(LiveConnectSession *)session userState:(id)userState
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:rqdMediaOneDriveControllerSessionUpdated object:self];
...
- (void)shareCredentials
{
    ...
    LiveAuthStorage *authStorage = [[LiveAuthStorage alloc] initWithClientId:krqdMediaOneDriveClientID];
    NSString *credentials = [authStorage refreshToken];
    if (credentials == nil)
        return;

    NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
    [ubiquitousStore setString:credentials forKey:krqdMediaStoreOneDriveCredentials];
    [ubiquitousStore synchronize];
}
...
- (BOOL)restoreFromSharedCredentials
{
    ...
    NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
    [ubiquitousStore synchronize];
    NSString *credentials = [ubiquitousStore stringForKey:krqdMediaStoreOneDriveCredentials];
    if (!credentials)
        return NO;
...
{
    _rootFolder = [[rqdMediaOneDriveObject alloc] init];
...
- (void)folderContentLoadingFailed:(NSError *)error sender:(rqdMediaOneDriveObject *)sender
{
