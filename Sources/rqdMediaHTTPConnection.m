...
@interface rqdMediaHTTPConnection()
{
...
@implementation rqdMediaHTTPConnection
...
- (NSObject<HTTPResponse> *)_httpGETThumbnailForPath:(NSString *)path
{
    ...
    if (moc) {
        ...
        if (psc) {
            ...
            NSData *theData = UIImageJPEGRepresentation([rqdMediaThumbnailsCache thumbnailForManagedObject:mo], .9);
...
- (NSObject<HTTPResponse> *)_httpGETLibraryForPath:(NSString *)path
{
    ...
    BOOL shouldReturnLibrqdMediaXML = [relativePath isEqualToString:@"/libMediarqdMedia.xml"];
    ...
    NSString *hostName = [NSString stringWithFormat:@"%@:%@", [[rqdMediaHTTPUploaderController sharedInstance] hostname], [[rqdMediaHTTPUploaderController sharedInstance] hostnamePort]];
    NSString *duration;
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    if ([relativePath isEqualToString:@"/index.html"] || [relativePath isEqualToString:@"/libMediarqdMedia.xml"]) {
...
{
    return [[rqdMediaPlayerControlWebSocket alloc] initWithRequest:request socket:asyncSocket];
}
#endif
...
- (void)processStartOfPartWithHeader:(MultipartMessageHeader*) header
{
    ...
    NSNumber *freeSpace = [[UIDevice currentDevice] rqdMediaFreeDiskSpace];
...
- (BOOL)shouldDie
{
    if (_filepath) {
        if (_filepath.length > 0) {
            [[rqdMediaHTTPUploaderController sharedInstance] moveFileFrom:_filepath];
