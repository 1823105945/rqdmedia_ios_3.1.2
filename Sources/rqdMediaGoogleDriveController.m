/*****************************************************************************
 * rqdMediaGoogleDriveController.m
 * rqdMedia for iOS
 *****************************************************************************
...
@interface rqdMediaGoogleDriveController ()
{
...
@implementation rqdMediaGoogleDriveController

#pragma mark - session handling

+ (instancetype)sharedInstance
{
    static rqdMediaGoogleDriveController *sharedInstance = nil;
    static dispatch_once_t pred;

    dispatch_once(&pred, ^{
        sharedInstance = [rqdMediaGoogleDriveController new];
    });
...
- (void)logout
{
    ...
    [ubiquitousStore setString:nil forKey:krqdMediaStoreGDriveCredentials];
...
- (void)shareCredentials
{
    ...
    NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
    [ubiquitousStore setString:credentials forKey:krqdMediaStoreGDriveCredentials];
    [ubiquitousStore synchronize];
}
...
- (BOOL)restoreFromSharedCredentials
{
    NSUbiquitousKeyValueStore *ubiquitousStore = [NSUbiquitousKeyValueStore defaultStore];
    [ubiquitousStore synchronize];
    NSString *credentials = [ubiquitousStore stringForKey:krqdMediaStoreGDriveCredentials];
    if (!credentials)
        return NO;
