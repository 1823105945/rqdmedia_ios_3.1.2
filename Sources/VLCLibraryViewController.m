...
#import "rqdMediaFolderCollectionViewFlowLayout.h"
#import "VLCMediaDataSource.h"
#import "VLCLibrarySearchDisplayDataSource.h"
#import "rqdMediaMovieViewController.h"
#import "rqdMediaPlaylistCollectionViewCell.h"
#import "rqdMediaPlaylistTableViewCell.h"
...
{
    UIViewController *firstStepsVC = [[rqdMediaFirstStepsViewController alloc] initWithNibName:nil bundle:nil];
...
@interface VLCLibraryViewController () <rqdMediaFolderCollectionViewDelegateFlowLayout, LXReorderableCollectionViewDataSource, LXReorderableCollectionViewDelegateFlowLayout, UITableViewDataSource, UITableViewDelegate, MLMediaLibrary, VLCMediaListDelegate, UISearchResultsUpdating, UISearchControllerDelegate> {
    VLCLibraryMode _libraryMode;
    VLCLibraryMode _previousLibraryMode;
    UIBarButtonItem *_menuButton;
    NSMutableArray *_indexPaths;
    id _folderObject;
    rqdMediaFolderCollectionViewFlowLayout *_folderLayout;
...
- (void)loadView
{
    ...
    self.emptyLibraryView = [[[NSBundle mainBundle] loadNibNamed:@"rqdMediaEmptyLibraryView" owner:self options:nil] lastObject];
    ...
    if (@available(iOS 11.0, *)) {
        dragAndDropManager = [rqdMediaDragAndDropManager new];
        ((rqdMediaDragAndDropManager *)dragAndDropManager).delegate = _mediaDataSource;
        UIDropInteraction *dropInteraction = [[UIDropInteraction alloc] initWithDelegate:(rqdMediaDragAndDropManager *)dragAndDropManager];
...
- (void)setupContentView
{
    ...
    contentView.backgroundColor = [UIColor rqdMediaDarkBackgroundColor];
    ...
    if (!_tableView) {
        ...
        topView.backgroundColor = [UIColor rqdMediaDarkBackgroundColor];
        [_tableView addSubview:topView];
        _tableView.rowHeight = [rqdMediaPlaylistTableViewCell heightOfCell];
        _tableView.separatorColor = [UIColor rqdMediaDarkBackgroundColor];
        _tableView.delegate = self;
        _tableView.dataSource = self;
        _tableView.indicatorStyle = UIScrollViewIndicatorStyleWhite;
        _tableView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        if (@available(iOS 11.0, *)) {
            _tableView.dragDelegate = ((rqdMediaDragAndDropManager *)dragAndDropManager);
            _tableView.dropDelegate = ((rqdMediaDragAndDropManager *)dragAndDropManager);
        } else {
            _tableView.tableHeaderView = _searchController.searchBar;
        }
        UINib *nib = [UINib nibWithNibName:@"rqdMediaPlaylistTableViewCell" bundle:nil];
    ...
    if (!_collectionView) {
        _folderLayout = [[rqdMediaFolderCollectionViewFlowLayout alloc] init];
        ...
        if (@available(iOS 11.0, *)) {
            _collectionView.dragDelegate = ((rqdMediaDragAndDropManager *)dragAndDropManager);
            _collectionView.dropDelegate = ((rqdMediaDragAndDropManager *)dragAndDropManager);
        }
        _collectionView.indicatorStyle = UIScrollViewIndicatorStyleWhite;
        _collectionView.delegate = self;
        _collectionView.dataSource = self;
        _collectionView.backgroundColor = [UIColor rqdMediaDarkBackgroundColor];
        ...
        [_collectionView registerNib:[UINib nibWithNibName:@"rqdMediaPlaylistCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"PlaylistCell"];
    }
...
- (void)openMediaObject:(NSManagedObject *)mediaObject
{
    if ([mediaObject isKindOfClass:[MLAlbum class]] || [mediaObject isKindOfClass:[MLShow class]]) {
        ...
        vpc.channels=[_mediaDataSource allObjects];
...
- (void)createSpotlightItem:(nonnull NSManagedObject *)mediaObject
{
    if (![rqdMediaKeychainCoordinator passcodeLockEnabled]) {
        ...
        self.userActivity = [[NSUserActivity alloc] initWithActivityType:krqdMediaUserActivityPlaying];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    rqdMediaPlaylistTableViewCell *cell = (rqdMediaPlaylistTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
...
{
    rqdMediaPlaylistTableViewCell *playlistTableViewCell = (rqdMediaPlaylistTableViewCell *)[tableView cellForRowAtIndexPath:indexPath];
    if ([playlistTableViewCell isExpanded]) {
        [playlistTableViewCell collapsWithAnimation:YES];
        return;
    }

    for (rqdMediaPlaylistTableViewCell *cell in [tableView visibleCells]) {
        if ([cell isExpanded])
    ...
    rqdMediaPlaybackController *vpc = [rqdMediaPlaybackController sharedInstance];
    vpc.currentPlayChannel=indexPath.row;
    vpc.channels=[_mediaDataSource allObjects];
    [vpc configureMediaListWithFiles:[_mediaDataSource allObjects] indexToPlay:indexPath.row];
...
- (void)swipeRightOnCollectionViewCellGestureAction:(UIGestureRecognizer *)recognizer
{
    ...
    rqdMediaPlaylistCollectionViewCell *cell = (rqdMediaPlaylistCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:path];
...
{
    rqdMediaPlaylistCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PlaylistCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.editing) {
        ...
        [(rqdMediaPlaylistCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath] selectionUpdate];
        return;
    }
    ...
    for (NSUInteger x = 0; x < cellCount; x++) {
        rqdMediaPlaylistCollectionViewCell *cell = visibleCells[x];
...
- (void)collectionView:(UICollectionView *)collectionView requestToMoveItemAtIndexPath:(NSIndexPath *)itemPath intoFolderAtIndexPath:(NSIndexPath *)folderPath
{
    ...
    if (!validFileTypeAtFolderPath) {
        rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"FOLDER_INVALID_TYPE_TITLE", nil) message:NSLocalizedString(@"FOLDER_INVALID_TYPE_MESSAGE", nil) cancelButtonTitle:nil otherButtonTitles:@[NSLocalizedString(@"BUTTON_OK", nil)]];
...
{
    rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"FOLDER_CHOOSE_NAME_TITLE", nil) message:NSLocalizedString(@"FOLDER_CHOOSE_NAME_MESSAGE", nil) cancelButtonTitle:NSLocalizedString(@"BUTTON_CANCEL", nil) otherButtonTitles:@[NSLocalizedString(@"BUTTON_SAVE", nil)]];
    ...
    __weak rqdMediaAlertView *weakAlert = alert;
    alert.completion = ^(BOOL cancelled, NSInteger buttonIndex) {
        if (cancelled)
            [self updateViewContents];
        else
...
- (void)createFolderWithName:(NSString *)folderName
{
    ...
    for (MLLabel *label in labels){
        if ([label.name isEqualToString:folderName]) {
            _folderObject = nil;
            _indexPaths = nil;
            rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"FOLDER_NAME_DUPLICATE_TITLE", nil) message:NSLocalizedString(@"FOLDER_NAME_DUPLICATE_MESSAGE", nil) cancelButtonTitle:nil otherButtonTitles:@[NSLocalizedString(@"BUTTON_OK", nil)]];
...
- (void)handleSelection
{
    if (self.usingTableViewToShowData) {
        ...
        for (NSInteger item = 0; item < numberOfItems; item++) {
            ...
            [(rqdMediaPlaylistCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath] selectionUpdate];
        }
    }
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated
{
    ...
    [visibleCells enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        rqdMediaPlaylistCollectionViewCell *aCell = (rqdMediaPlaylistCollectionViewCell*)obj;
...
- (void)renameSelection
{
    ...
    NSString *itemName;
    if (self.usingTableViewToShowData)
        itemName = [(rqdMediaPlaylistTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPaths[0]] titleLabel].text;
    else
        itemName = [(rqdMediaPlaylistCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPaths[0]] titleLabel].text;

    rqdMediaAlertView *alert = [[rqdMediaAlertView alloc] initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"RENAME_MEDIA_TO", nil), itemName] message:nil cancelButtonTitle:NSLocalizedString(@"BUTTON_CANCEL", nil) otherButtonTitles:@[NSLocalizedString(@"BUTTON_RENAME", nil)]];
    [alert setAlertViewStyle:UIAlertViewStylePlainTextInput];
...
- (void)share:(UIBarButtonItem *)barButtonItem
{
    ...
    UIActivityViewController *controller = [rqdMediaActivityViewControllerVendor activityViewControllerForFiles:[self fileURLsFromSelection] presentingButton:barButtonItem presentingViewController:self];
    if (!controller) {
