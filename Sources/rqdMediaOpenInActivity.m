...
@interface rqdMediaOpenInActivity () <UIDocumentInteractionControllerDelegate>
@end

@implementation rqdMediaOpenInActivity
{
...
- (void)performActivity
{
    ...
    if (count > 1) {
        ...
        rqdMediaAlertView *alertView = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"SHARING_ERROR_NO_FILES", nil)
                                                              message:nil
                                                             delegate:nil
...
- (void)presentDocumentInteractionControllerWithFileURL:(NSURL *)fileURL
{
    ...
    dispatch_block_t controllerPresentationBlock = ^{
        ...
        if (!controllerWasPresentedSuccessfully) {
            rqdMediaAlertView *alertView = [[rqdMediaAlertView alloc] initWithTitle:NSLocalizedString(@"SHARING_ERROR_NO_APPLICATIONS", nil)
                                                                  message:nil
                                                                 delegate:nil
                                                        cancelButtonTitle:NSLocalizedString(@"BUTTON_OK", nil)
                                                        otherButtonTitles:nil];
