...
@objc protocol rqdMediaActionSheetDataSource {
...
@objc protocol rqdMediaActionSheetDelegate {
...
class rqdMediaActionSheet: UIViewController {

    private let cellHeight: CGFloat = 50

    @objc weak var dataSource: rqdMediaActionSheetDataSource?
    @objc weak var delegate: rqdMediaActionSheetDelegate?
    ...
    lazy var collectionView: UICollectionView = {
        ...
        collectionView.backgroundColor = .rqdMediaDarkBackground()
        collectionView.alwaysBounceVertical = true
        collectionView.showsVerticalScrollIndicator = false
        collectionView.register(rqdMediaActionSheetCell.self, forCellWithReuseIdentifier: rqdMediaActionSheetCell.identifier)
        collectionView.translatesAutoresizingMaskIntoConstraints = false
        return collectionView
    }()

    lazy var headerView: rqdMediaActionSheetSectionHeader = {
        let headerView = rqdMediaActionSheetSectionHeader()
        headerView.title.text = delegate?.headerViewTitle?() ?? "Default header title"
        headerView.title.textColor = .white
        headerView.backgroundColor = .rqdMediaDarkBackground()
    ...
    lazy var bottomBackgroundView: UIView = {
        ...
        bottomBackgroundView.backgroundColor = .rqdMediaDarkBackground()
    ...
    fileprivate lazy var collectionViewHeightConstraint: NSLayoutConstraint = {
        guard let dataSource = dataSource else {
            preconditionFailure("rqdMediaActionSheet: Data source not set correctly!")
        }
...
extension rqdMediaActionSheet: UICollectionViewDelegateFlowLayout {
...
extension rqdMediaActionSheet: UICollectionViewDelegate {
...
extension rqdMediaActionSheet: UICollectionViewDataSource {
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if let dataSource = dataSource {
            return dataSource.numberOfRows()
        }
        preconditionFailure("rqdMediaActionSheet: No data source")
    }
