...
- (void)viewDidLoad
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"rqdMediaTVHProgramDetailCell" bundle:nil] forCellReuseIdentifier:@"ProgramDetailViewTableItem"];
...
- (IBAction)playStream:(id)sender {
    ...
    VLCMedia *media = [VLCMedia mediaWithURL:[NSURL URLWithString: [[self.dvrItem streamUrlWithTranscoding:YES withInternal:NO] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]]];
    [self.medialist addMedia:media];
    [rqdMediaPlaybackController sharedInstance].dvrItem=self.dvrItem;
...
{
    rqdMediaTVHProgramDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ProgramDetailViewTableItem" forIndexPath:indexPath];
    __weak typeof(self)SeftWeak=self;
    cell.Clock = ^(id sender) {
        [SeftWeak addRecordMoreItemsToTVHeadend:sender];
    };
    TVHEpg *epg;
