...
#import "CAAnimation+rqdMediaWiggle.h"

#define remotePlaybackReuseIdentifer @"remotePlaybackReuseIdentifer"

@interface rqdMediaRemotePlaybackViewController () <UICollectionViewDataSource, UICollectionViewDelegate, VLCMediaFileDiscovererDelegate>
{
...
@implementation rqdMediaRemotePlaybackViewController
...
- (void)viewDidLoad
{
    ...
    [self.cachedMediaCollectionView registerNib:[UINib nibWithNibName:@"rqdMediaRemoteBrowsingTVCell" bundle:nil]
...
- (void)updateHTTPServerAddress
{
    ...
    NSString *uploadText = connectedViaWifi ? [[rqdMediaHTTPUploaderController sharedInstance] httpStatus] : NSLocalizedString(@"HTTP_UPLOAD_NO_CONNECTIVITY", nil);
    self.httpServerLabel.text = uploadText;
    if (connectedViaWifi && [rqdMediaHTTPUploaderController sharedInstance].isServerRunning)
...
{
    BOOL futureHTTPServerState = ![rqdMediaHTTPUploaderController sharedInstance].isServerRunning ;
    [[NSUserDefaults standardUserDefaults] setBool:futureHTTPServerState forKey:krqdMediaSettingSaveHTTPUploadServerStatus];
    [[rqdMediaHTTPUploaderController sharedInstance] changeHTTPServerState:futureHTTPServerState];
    [self updateHTTPServerAddress];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(rqdMediaRemoteBrowsingTVCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
    NSString *cellTitle;
...
- (void)setEditing:(BOOL)editing
{
    ...
    if (editing) {
        [focusedCell.layer addAnimation:[CAAnimation rqdmedia_wiggleAnimationwithSoftMode:NO]
                                 forKey:rqdMediaWiggleAnimationKey];
    } else {
        [focusedCell.layer removeAnimationForKey:rqdMediaWiggleAnimationKey];
    }
}
