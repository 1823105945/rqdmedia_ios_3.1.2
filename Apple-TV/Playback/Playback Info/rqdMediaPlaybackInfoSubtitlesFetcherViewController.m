...
@interface rqdMediaPlaybackInfoSubtitlesFetcherViewController () <UITableViewDataSource, UITableViewDelegate, MDFOSOFetcherDataRecipient>
{
...
@implementation rqdMediaPlaybackInfoSubtitlesFetcherViewController
...
- (void)viewDidLoad {
    ...
    NSString *selectedLocale = [defaults stringForKey:krqdMediaSettingLastUsedSubtitlesSearchLanguage];
    if (!selectedLocale) {
        ...
        selectedLocale = [preferredLanguage rqdMediathreeLetterLanguageKeyForTwoLetterCode];
        /* last resort */
        if (selectedLocale == nil) {
            selectedLocale = @"eng";
        }
        [defaults setObject:selectedLocale forKey:krqdMediaSettingLastUsedSubtitlesSearchLanguage];
    }
...
- (void)viewWillAppear:(BOOL)animated
{
    if ([UIScreen mainScreen].traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
        ...
        self.titleLabel.textColor = [UIColor rqdMediaLightTextColor];
    } else {
        self.visualEffectView.effect = [UIBlurEffect effectWithStyle:UIBlurEffectStyleLight];
        self.titleLabel.textColor = [UIColor rqdMediaDarkTextColor];
    }
...
- (void)searchForMedia
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    _osoFetcher.subtitleLanguageId = [defaults stringForKey:krqdMediaSettingLastUsedSubtitlesSearchLanguage];
...
- (void)MDFOSOFetcher:(MDFOSOFetcher *)aFetcher subtitleDownloadSucceededForItem:(MDFSubtitleItem *)subtitleItem atPath:(NSString *)pathToFile
{
    ...
    [self dismissViewControllerAnimated:YES completion:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:rqdMediaPlaybackControllerPlaybackMetadataDidChange object:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section != 0) {
        ...
        NSString *selectedLocale = [[NSUserDefaults standardUserDefaults] objectForKey:krqdMediaSettingLastUsedSubtitlesSearchLanguage];
        cell.textLabel.text = NSLocalizedString(@"LANGUAGE", nil);
        NSString *detail = [[selectedLocale rqdMediatwoLetterLanguageKeyForThreeLetterCode] rqdMedialocalizedLanguageNameForTwoLetterCode];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        NSString *currentCode = [defaults stringForKey:krqdMediaSettingLastUsedSubtitlesSearchLanguage];
        ...
        for (MDFSubtitleLanguage *item in languages) {
            ...
            UIAlertAction *action = [UIAlertAction actionWithTitle:item.localizedName
                                                           ...
                                                           handler:^(UIAlertAction * _Nonnull action) {
                                                               ...
                                                               [defaults setObject:itemID forKey:krqdMediaSettingLastUsedSubtitlesSearchLanguage];
                                                               [self searchForMedia];
                                                               [self.tableView reloadData];
                                                           }];
