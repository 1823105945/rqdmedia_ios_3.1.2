/*****************************************************************************
 * rqdMediaDragAndDropManager.swift
 * rqdMedia for iOS
 *****************************************************************************
...
@available(iOS 11.0, *)
@objc protocol rqdMediaDragAndDropManagerDelegate : NSObjectProtocol {
...
@available(iOS 11.0, *)
class rqdMediaDragAndDropManager: NSObject, UICollectionViewDragDelegate, UITableViewDragDelegate, UICollectionViewDropDelegate, UITableViewDropDelegate, UIDropInteractionDelegate
{
    @objc weak var delegate:rqdMediaDragAndDropManagerDelegate?

    let utiTypeIdentifiers:[String] = rqdMediaDragAndDropManager.supportedTypeIdentifiers()

    /// Returns the supported type identifiers that rqdMedia can process.
    ...
    func tableView(_ tableView: UITableView, performDropWith coordinator: UITableViewDropCoordinator) {
        ...
        for item in coordinator.items {
            ...
            if item.sourceIndexPath != nil { //element within rqdMedia
                moveItem(tableView:tableView, item:item, toIndexPath:destinationPath)
                continue
            }
            //Element dragging from another App
            let placeholder = UITableViewDropPlaceholder(insertionIndexPath: destinationPath, reuseIdentifier: "PlaylistCell", rowHeight: rqdMediaPlaylistTableViewCell.heightOfCell())
