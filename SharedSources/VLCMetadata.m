...
- (void)updateMetadataFromMediaPlayer:(VLCMediaPlayer *)mediaPlayer;
{
    ...
    if ([rqdMediaPlaybackController sharedInstance].mediaList) {
    ...
    if (item) {
        ...
        if (metaDict) {
            self.title =  metaDict[VLCMetaInformationTitle];
    ...
    if (self.isAudioOnly) {
#if TARGET_OS_IOS
        self.artworkImage = [rqdMediaThumbnailsCache thumbnailForManagedObject:item];
...
#if TARGET_OS_IOS
    if ([rqdMediaKeychainCoordinator passcodeLockEnabled]) return;
#endif
