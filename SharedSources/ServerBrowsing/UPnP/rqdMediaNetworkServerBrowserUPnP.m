/*****************************************************************************
 * rqdMediaNetworkServerBrowserUPnP.m
 * rqdMedia for iOS
 *****************************************************************************
...
#import "BasicUPnPDevice+rqdMedia.h"

@interface rqdMediaNetworkServerBrowserUPnP ()
...
@implementation rqdMediaNetworkServerBrowserUPnP
...
- (instancetype)initWithUPNPDevice:(MediaServer1Device *)device header:(NSString *)header andRootID:(NSString *)upnpRootID
{
    ...
    if (self) {
        ...
        _upnpQueue.name = @"org.videolan.rqdmedia-ios.upnp.update";
...
- (void)update {
    [self.upnpQueue addOperationWithBlock:^{
        ...
        for (MediaServer1BasicObject *object in objectsArray) {
            [itemsArray addObject:[[rqdMediaNetworkServerBrowserItemUPnP alloc] initWithBasicObject:object device:self.upnpDevice]];
        }
...
- (VLCMediaList *)buildMediaList
{
    ...
    @synchronized(_items) {
        ...
        for (id<rqdMediaNetworkServerBrowserItem> item in _items) {
...
@interface MediaServer1ItemObject (rqdMedia)
@end

@implementation MediaServer1ItemObject (rqdMedia)

- (id)rqdmedia_ressourceItemForKey:(NSString *)key urlString:(NSString *)urlString device:(MediaServer1Device *)device {
    ...
    NSRange position = [key rangeOfString:krqdMediaUPnPVideoProtocolKey];
    ...
    if ([device rqdMedia_isHDHomeRunMediaServer]) {
    ...
    return [[rqdMediaNetworkServerBrowserItemUPnPMultiRessource alloc] initWithTitle:profileTitle url:[NSURL URLWithString:urlString]];
}

- (NSArray *)rqdmedia_ressourceItemsForDevice:(MediaServer1Device *)device {
    ...
    [uriCollection enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key, NSString *  _Nonnull urlString, BOOL * _Nonnull stop) {
        id item = [self rqdmedia_ressourceItemForKey:key urlString:urlString device:device];
        if (item) {
...
@interface rqdMediaNetworkServerBrowserItemUPnP ()
@property (nonatomic, readonly) MediaServer1BasicObject *mediaServerObject;
@property (nonatomic, readonly) MediaServer1Device *upnpDevice;

@end

@implementation rqdMediaNetworkServerBrowserItemUPnP
        ...
        if (!_container && [basicObject isKindOfClass:[MediaServer1ItemObject class]]) {
            ...
            protocolStrings = [protocolStrings filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(NSString * _Nonnull evaluatedObject, NSDictionary<NSString *,id> * _Nullable bindings) {
                ...
                if ([evaluatedObject respondsToSelector:@selector(containsString:)]) {
                    if ([evaluatedObject containsString:krqdMediaUPnPVideoProtocolKey])
                        return YES;
                    if ([evaluatedObject containsString:krqdMediaUPnPAudioProtocolKey])
                        return YES;
                } else {
                    NSRange foundRange = [evaluatedObject rangeOfString:krqdMediaUPnPVideoProtocolKey];
                    if (foundRange.location != NSNotFound)
                        return YES;
                    foundRange = [evaluatedObject rangeOfString:krqdMediaUPnPAudioProtocolKey];
                    if (foundRange.location != NSNotFound)
                        return YES;
                }
                return NO;
            ...
            if ([mediaItem.uriCollection count] > 1) {
                for (NSString *key in mediaItem.uriCollection) {
                    if ([key respondsToSelector:@selector(containsString:)]) {
                        if ([key containsString:krqdMediaUPnPVideoProtocolKey] || [key containsString:krqdMediaUPnPAudioProtocolKey]) {
                            mediaItem.uri = [mediaItem.uriCollection objectForKey:key];
                        }
                    } else {
                        NSRange foundRage = [key rangeOfString:krqdMediaUPnPVideoProtocolKey];
                        if (foundRage.location != NSNotFound) {
                            mediaItem.uri = [mediaItem.uriCollection objectForKey:key];
                        } else {
                            foundRage = [key rangeOfString:krqdMediaUPnPAudioProtocolKey];
                            if (foundRage.location != NSNotFound) {
                                mediaItem.uri = [mediaItem.uriCollection objectForKey:key];
                            }
                        }
...
- (BOOL)isDownloadable {
    // Disable downloading for the HDHomeRun for now to avoid infinite downloads (URI needs a duration parameter, otherwise you are just downloading a live stream). rqdMedia also needs an extension in the file name for this to work.
    BOOL downloadable = ![self.upnpDevice rqdMedia_isHDHomeRunMediaServer];
    return downloadable;
}
...
- (id<rqdMediaNetworkServerBrowser>)containerBrowser {
    ...
    if (basicObject.isContainer) {
        return [[rqdMediaNetworkServerBrowserUPnP alloc] initWithUPNPDevice:self.upnpDevice header:self.mediaServerObject.title andRootID:self.mediaServerObject.objectID];
    } else if ([basicObject isKindOfClass:[MediaServer1ItemObject class]]) {
        return [[rqdMediaNetworkServerBrowserUPnPMultiRessource alloc] initWithItem:(MediaServer1ItemObject *)self.mediaServerObject device:self.upnpDevice];
    } else {
        return nil;
    }
}
...
@implementation rqdMediaNetworkServerBrowserUPnPMultiRessource
...
- (instancetype)initWithItem:(MediaServer1ItemObject *)itemObject device:(MediaServer1Device *)device
{
    ...
    if (self) {
        ...
        _items = [itemObject rqdmedia_ressourceItemsForDevice:device];
        _mediaList = [self buildMediaList];
    }
    return self;
}
...
@implementation rqdMediaNetworkServerBrowserItemUPnPMultiRessource
