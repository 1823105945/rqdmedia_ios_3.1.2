/*****************************************************************************
 * rqdMediaLocalNetworkServiceBrowserUPnP.m
 * rqdMedia for iOS
 *****************************************************************************
...
@interface rqdMediaLocalNetworkServiceBrowserUPnP () <UPnPDBObserver>{
    BOOL _udnpDiscoveryRunning;
    NSTimer *_searchTimer;
    BOOL _setup;
}
@property (nonatomic) NSArray<rqdMediaLocalNetworkServiceUPnP*> *filteredUPNPDevices;
@property (nonatomic) NSArray *UPNPdevices;
@end

@implementation rqdMediaLocalNetworkServiceBrowserUPnP
...
#pragma mark - rqdMediaLocalNetworkServiceBrowser Protocol
...
- (void)_startUPNPDiscovery
{
    ...
    if (!_setup) {
        [[managerInstance SSDP] setUserAgentProduct:[NSString stringWithFormat:@"rqdMediaforiOS/%@", [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"]] andOS:[NSString stringWithFormat:@"iOS/%@", [[UIDevice currentDevice] systemVersion]]];
        _setup = YES;
    }
...
- (void)UPnPDBUpdated:(UPnPDB*)sender
{
    ...
    NSMutableArray<rqdMediaLocalNetworkServiceUPnP*> *mutArray = [[NSMutableArray alloc] init];
    for (NSUInteger x = 0; x < count; x++) {
        device = _UPNPdevices[x];
        if ([[device urn] isEqualToString:@"urn:schemas-upnp-org:device:MediaServer:1"])
            [mutArray addObject:[[rqdMediaLocalNetworkServiceUPnP alloc] initWithUPnPDevice:device serviceName:self.name]];
        else
