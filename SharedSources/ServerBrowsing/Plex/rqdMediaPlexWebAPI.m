...
@implementation rqdMediaPlexWebAPI
...
- (BOOL)PlexCreateIdentification:(NSString *)username password:(NSString *)password
{
    ...
    [request setValue:@"PlexrqdMedia" forHTTPHeaderField:@"X-Plex-Product"];
    [request setValue:appVersion forHTTPHeaderField:@"X-Plex-Version"];
    [request setValue:@"rqdMedia for iOS" forHTTPHeaderField:@"X-Plex-Device-Name"];
...
- (NSString *)PlexAuthentification:(NSString *)username password:(NSString *)password
{
    ...
    if ((![username isEqualToString:@""]) && (![password isEqualToString:@""])) {
        ...
        rqdMediaPlexParser *plexParser = [[rqdMediaPlexParser alloc] init];
...
- (NSInteger)MarkWatchedUnwatchedMedia:(NSString *)adress port:(NSString *)port videoRatingKey:(NSString *)ratingKey state:(NSString *)state authentification:(NSString *)auth
{
    ...
    NSURLRequest *request = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:[[[rqdMediaPlexWebAPI alloc] init] urlAuth:url authentification:auth]] cachePolicy:NSURLRequestReloadIgnoringLocalAndRemoteCacheData timeoutInterval:20];
...
- (NSString *)getFileSubtitleFromPlexServer:(NSDictionary *)mediaObject modeStream:(BOOL)modeStream error:(NSError *__autoreleasing *)outError
{
    ...
    rqdMediaPlexWebAPI *PlexWebAPI = [[rqdMediaPlexWebAPI alloc] init];
    ...
    if (receivedSub.length < [[UIDevice currentDevice] rqdMediaFreeDiskSpace].longLongValue) {
        ...
        if (outError) {
            *outError = [NSError errorWithDomain:@"org.videolan.rqdmedia-ios.plex"
                                            code:EX_CANTCREAT
