/*****************************************************************************
 * rqdMediaNetworkServerBrowserFTP.m
 * rqdMedia for iOS
 *****************************************************************************
...
@interface rqdMediaNetworkServerBrowserFTP () <WRRequestDelegate>
@property (nonatomic) NSURL *url;
@property (nonatomic) WRRequestListDirectory *FTPListDirRequest;

@end

@implementation rqdMediaNetworkServerBrowserFTP
    ...
    if (request == _FTPListDirRequest) {
        ...
        for (NSUInteger x = 0; x < count; x++) {
            ...
            if (![filename hasPrefix:@"."])
            {
                ...
                [filteredList addObject:[[rqdMediaNetworkServerBrowserItemFTP alloc] initWithDictionary:dict baseURL:self.url subtitleURL:subtitleURL]];
            }
        }
...
@implementation rqdMediaNetworkServerBrowserItemFTP
...
- (instancetype)initWithDictionary:(NSDictionary *)dict baseURL:(NSURL *)baseURL subtitleURL:(NSURL *)subtitleURL
{
    ...
    if (self) {
        ...
        NSData *flippedData = [rawFileName dataUsingEncoding:[[[NSUserDefaults standardUserDefaults] objectForKey:krqdMediaSettingFTPTextEncoding] intValue] allowLossyConversion:YES];
        if (flippedData != nil) {
...
- (id<rqdMediaNetworkServerBrowser>)containerBrowser {
    return [[rqdMediaNetworkServerBrowserFTP alloc] initWithURL:self.URL];
}
